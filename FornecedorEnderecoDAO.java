...
public class FornecedorEnderecoDAO {
    ...
    public static ArrayList<Endereco> retreaveAll(int fk_fornecedor) throws SQLException {

        Connection conn = BancoDados.createConnection();

        ResultSet rs = conn.createStatement().executeQuery(
                "select * from fornecedor_endereco where fk_fornecedor =" + fk_fornecedor
        );
        ...
        while (rs.next()) {
            aux.add(new Endereco(
                    ...
                    rs.getString("pais"),
                    rs.getString("cep"),
                    rs.getInt("pk_fornecedor_endereco"),
                    fk_fornecedor));
        }
    ...
    public static void update(Endereco e)throws SQLException{
        ...
        PreparedStatement stm = conn.prepareStatement(
             "UPDATE fornecedor_endereco SET logradouro=?, bairro=?, cidade=?, estado=?, pais=?, cep=? WHERE pk_fornecedor_endereco=?"
        );
    ...
    public static void delete(int pk_endereco) throws SQLException {
        ...
        PreparedStatement stm = conn.prepareStatement(
        "delete from fornecedor_endereco where pk_fornecedor_endereco = ?"
        );
